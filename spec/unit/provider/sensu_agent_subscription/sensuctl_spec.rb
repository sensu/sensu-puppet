...
describe Puppet::Type.type(:sensu_agent_subscription).provider(:sensuctl) do
  let(:provider) { described_class }
  let(:resource) { Puppet::Type.type(:sensu_agent_subscription).new({name: 'test', entity: 'agent'}) }
  ...
  describe 'self.instances' do
    ...
    it 'should return the resource for a entity' do
      ...
      expect(property_hash[:name]).to eq('entity:sensu-backend.example.com on sensu-backend.example.com in default')
    end
  end
