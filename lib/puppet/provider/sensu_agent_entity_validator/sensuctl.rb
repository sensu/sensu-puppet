...
Puppet::Type.type(:sensu_agent_entity_validator).provide(:sensuctl, :parent => Puppet::Provider::Sensuctl) do
  desc "A provider for the resource type `sensu_agent_entity_validator`"

  def validate
    ret = execute(['sensuctl', 'entity', 'info', resource[:name], '--namespace', resource[:namespace]], failonfail: false)
